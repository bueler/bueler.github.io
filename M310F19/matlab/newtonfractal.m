...
% using intersection of two curves in plane:
%   x^3 - 3 x y^2 + 1 = 0
%   3 x^2 y - y^3     = 0
...
xrange = linspace(-2,2,N);  % generate NxN grid
...
axis equal
xlabel x,  ylabel y
title('RGB dots mark intersections of curves f_i(x,y)=0')

% zz(k,l) is 0,1,2,3:
%   0 if root not identified
%   1 if (-1,0)
%   2 if (1/2,sqrt(3)/2)
%   3 if (1/2,-sqrt(3)/2)
zz = zeros(size(xx));
