...
\docdate{Wednesday 24 April 2019}
...
Newton's method solves a problem
    $$f(x)=0$$
by starting with an initial estimate $x_0$.  It gets a new estimate from the old estimate by finding where the linearization $L(x) \approx f(x)$ crosses the $x$-axis.  (\emph{Write down the linearization $L(x)$ at $a=x_0$.  Solve $L(x)=0$ for $x$.})  So it uses this formula:
