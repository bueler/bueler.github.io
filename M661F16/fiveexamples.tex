...
\end{enumerate}
While the textbook\footnote{Nocedal \& Wright, \emph{Numerical Optimization}, 2nd ed., 2006} does not give any fleshed-out examples, the authors and other such experts have been exposed to many of them.

On goal 2), your method may be ``brute force'' or inefficient \dots \emph{that is just fine!}  The textbook and the rest of the content of the course will make more sense if you see that brute force approaches are the starting point of later elegant algorithms.

Assignment \#1 is based on these examples.  (Later Assignments will be largely based on textbook exercises, but I'll add my own problems and examples when needed.)  Note that each example has a ``code name'' like ``\texttt{fit}'' which I will use to name my corresponding \Matlab code (``\texttt{fit.m}'') when I hand out solutions to A \#1.
...
This problem is not in standard form (1.1), but an easy exercise on A \#1 asks you to put all of the inequality constraints in the form ``$c_i(x) \ge 0$'' to make it so.
...
\medskip
\item (\texttt{beam})  \quad A new tent design requires bending a pole to go from one corner of the tent to another while also having certain heights at particular distances.  Note that the corners of the tent are on the ground.  We want to know the shape of the pole, which is modeled as a \emph{beam} in standard engineering language, so that the fabric can be cut to reach the pole and be taut below it.

To be specific, the length of the tent along the ground is $\pi$ length units.  We seek a function $y=h(x)$, defined on the interval $x\in[0,\pi]$, for the height of the pole above the ground.  This function must be at least twice-differentiable and satisfy $h(0)=0$ and $h(\pi)=0$.  The theory of beams says that actual shape will be the minimizer
    $$\min_h I[h]$$
where
    $$I[h] = \frac{1}{2} \int_0^\pi |h''(x)|^2\,dx$$
is called the objective \emph{functional} because its input $h$ is itself a function from an (infinite-dimensional) vector space of functions.

The following inequalities give locations where the heights are constrained:
\begin{align*}
0.9 &\le h(1) \le 1.1 \\
1.2 &\le h(2) \le 1.4 \\
0.4 &\le h(3) \le 0.6
\end{align*}
The figure below does not show the shape of the tent pole, i.e.~it does not show the \emph{solution} of the minimization problem, but it does show a \emph{feasible} $h(x)$ which has the right differentiability and which satisfies the (inequality) constraints.
