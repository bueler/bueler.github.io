...
\medskip
This is an example of the famous \emph{traveling salesperson problem}.  If $x$ denotes a valid (``feasible'') seven-letter string (itinerary) then we may define the objective function $f(x)$ to be the cost of that itinerary; thus $f(x)$ is defined using the edge weights.  Though finding a feasible itinerary is seemingly nontrivial, at least for complicated graphs, one may add in all remaining edges with large weights so that any itinerary $x$ is feasible with well-defined cost $f(x)$.
